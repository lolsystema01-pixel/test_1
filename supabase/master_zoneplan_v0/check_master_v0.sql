...
-- ① 件数（zone_plan=8 / address_master=14）-------------------
select 'zone_plan' as tbl, count(*) from public.zone_plan
union all
select 'address_master', count(*) from public.address_master;


-- ② 重複排除（同一キーが2件以上ない＝0行）--------------------
select common_id, count(*) from public.zone_plan      group by common_id having count(*) > 1;  -- 0行
select town_key,  count(*) from public.address_master group by town_key  having count(*) > 1;  -- 0行


-- ③ 判定経路：共通IDで Master × ZonePlan を結合 ---------------
...
where table_schema = 'public'
  and table_name in ('address_master','zone_plan')
  and column_name in ('version','is_valid')
order by table_name, column_name;


-- ⑦ 住所→ゾーンの土台（例：TownKeyからゾーン・隣接・拠点）----
