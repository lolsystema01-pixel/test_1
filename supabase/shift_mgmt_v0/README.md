...
| # | ファイル | 内容 |
|---|---|---|
| 1 | `work_schedules_ext_v0.sql` | `work_schedules` に `preferred_areas`(common_id[])/`is_virtual`/`is_absent` 追加（**default無し・既存不変**）＋希望エリア妥当性（`preferred_areas_ok`）＋重複警告 |
...
| 書き込み口 | 呼ぶ人 | 関数内の認可（門番） |
|---|---|---|
| `apply_shift` | driver | `my_driver()` 本人のみ・`request_period_days` 期間チェック・二重申請防止。**driver_id は引数で受けない**（なりすまし防止） |
| `approve_reject_shift` | area | `my_office_drivers()` 配下のみ・状態遷移（申請中→承認/却下） |
| `office_direct_shift` | area | 同上・承認状態で直接登録（フォールバック） |
...
node supabase/shift_mgmt_v0/pglite_test_definers.mjs         # 17/17（3関数の認可・期間/二重・遷移・headcount・範囲外0件）
```
