...
| ロール | 保存できる範囲 |
|---|---|
| **hq** | いつでも・全営業所（初回は `save_office_init_setup`、再編集は `/admin/settings` の `set_office_gdrive_url`） |
| **area** | **自営業所のみ**、かつ **`gdrive_folder_url` が NULL のとき（＝初回）だけ**（`save_office_init_setup`） |
...
| ファイル | 内容 |
|---|---|
| `office_init_setup_v0.sql` | `gdrive_folder_url` 追加＋URL検証 `is_valid_gdrive_url`＋保存口 `save_office_init_setup`（初回）／`set_office_gdrive_url`（hq再編集・NULLクリア可）。冪等 |
| `check_office_init_v0.sql` | 列・ゲート判定・権限の確認（§4 はなりすまし実証・rollback） |
| `pglite_test_office_init.mjs` | 56/56 PASS |
...
権限設計では「hq＝常時・全営業所」だが、当初 UI に欄が無く hq は生SQLでしか変更できなかった（レビュー MED-3）。
**本PRで `/admin/settings` に持出バッグリストURL欄を追加**して解決した。

- 保存は hq 専用 RPC **`set_office_gdrive_url(office, url)`**（`office_init_setup_v0.sql` §2-2）。
  `save_office_init_setup`（初回設定・両項目必須・NULL不可）とは別口にした理由:
  再編集では **URLだけ付け替えたい／未完に戻したい（NULLクリア）** が必要で、初回設定の口では表現できないため。
- 画面は printer_model 等と同じ保存ボタンで、`update_office_settings`（4項目）＋ `set_office_gdrive_url`（URL）を続けて呼ぶ。
- **空欄で保存＝未完に戻す**（誤登録・営業所閉鎖時）。その営業所は次回ログインで初期設定画面が再表示される。
- 検証: `admin/settings` は変更していない `update_office_settings` を壊さず（同シグネチャ）、
  gdrive は別RPC＝**既存モジュール（admin_settings_v0）の関数シグネチャを触らない**。

## フォローアップ課題（本PR範囲外）

- **URL検証の単一化は完了**（`is_valid_gdrive_url` に集約・CHECK＋両RPCが共有）。一方 **printer_model の
  許容値は依然2箇所ハードコード**（CHECK と `update_office_settings` 内 `not in`）＝admin_settings_v0 側の課題。
...
```bash
node supabase/office_init_setup_v0/pglite_test_office_init.mjs   # 56/56
