...
- **処理能力優先（既定）** ＝ capの大きい実ドライバーから、Phase1で主担当ゾーン1本→Phase2で**隣接ランク≤3**のゾーンを積み増しcap充填（割当後 ≤ cap）。
  - 隣接ランク: `zone_rank()` ＝ 1=同一ゾーン / 2=同一市（`area_master.municipality`）/ 3=隣接（`zone_plan.adjacent_zones`）/ 99=対象外（割り当てない）。
...
- 同一市判定(rank2)は **`area_master.municipality`** から導出（is_valid＋priority で決定化）。※旧 `address_master` は語彙是正⑤で撤去済み（2026-07-17）。関数の実体は vocab_fix_v0/migrate_functions_to_area_master_v0.sql。
