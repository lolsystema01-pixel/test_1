...
// ★テスト専用: 照合なし帯の挙動検証のため kaz のみ有効化する（superuser操作）。
//   実運用の初期値は kaz/a/four とも enabled=false（レビューMED-1対応）＝[2b]で実証。
await db.exec(`update public.number_bands set enabled = true where band_key = 'kaz'`);
...
console.log('\n[2) 帯判定（最長prefix優先。register_receptionと同一ロジックを直接照会・副作用なし）]');
{
  const matchBand = async (tn) => {
    const rows = (await db.query(
      `select band_key, verify_on_reception from public.number_bands
       where enabled and left($1, char_length(prefix)) = prefix
  ...
  // [2b] レビューMED-1対応: 仮値帯（a/four）は既定enabled=false＝照合されない（開放面なし）
  ok('★仮値帯A*は照合されない（enabled=false）', (await matchBand('A12345')) === null);
  ok('★仮値帯4*は照合されない（enabled=false）', (await matchBand('4001234567')) === null);
  const init = (await db.query(`select band_key, enabled from public.number_bands order by band_key`)).rows;
  ok('enabled初期値: demo9000/req/dsp=true・a/four=false（kazはテストで有効化済）',
    init.every((r) => (['a','four'].includes(r.band_key) ? r.enabled === false : r.enabled === true)));
}
...
await asAnon(async () => {
  const r = await reg(['KAZ40099999999', '置き配', null, null, '宅配ボックス', 'web', null, false]);
  ok('anonでもregister_receptionを実行できる（grant済み）', r.result === 'created' && r.band_key === 'kaz');
  // レビューMED-1対応: 停止中の仮値帯（4*）はanonから受付できない（開放面の閉鎖を実証）
  const closed = await reg(['4009999999', '置き配', null, null, '宅配ボックス', 'web', null, false]);
  ok('★停止中の仮値帯4*はformat_error（受付行を作れない）', closed.result === 'format_error');
  // reception_requestsへのSELECTはanonに与えていない仕様のため、確認は role をリセットして行う（同一トランザクション内・rollback前）。
  await db.exec('reset role');
  const row = (await db.query(`select created_by from public.reception_requests where tracking_number='KAZ40099999999'`)).rows[0];
