...
| `pglite_test.mjs` | E2E検証（**105/105 PASS**） |
...
初期状態: `demo9000`/`req`/`dsp` は照合あり。`kaz`/`a`/`four` は**仮値＝帯ごと停止（enabled=false）**
（レビューMED-1対応: 仮値のまま開けておくと anon が実在チェックなしの受付を量産できるため。
単字prefix `A`/`4` は実番号の誤照合リスクもある）。

現場確認後の解禁は2段の1行UPDATE——①帯を有効化（enabled=true）②必要なら実在チェックも有効化：

```sql
update public.number_bands set enabled = true where band_key = 'kaz';  -- ①帯の解禁
```
...
### ③ `unchanged` 判定は内容5項目比較（channel/caller_phoneは除外・memoは含む）
...
### ③b memo（自由記入）の保存（v0.2・LOL指摘対応）
現行フォームのmemoを `reception_requests.memo` に保存する（500字上限・関数I2チェック＋列CHECKの二段構え）。
自由記入＝PII混入がありうる前提で、**`get_reception_public`（非PII照会）には返さない**（源流強制）。
memoだけの変更も `overwritten`（旧行=取消の履歴保全）として扱う。
`register_reception` は第9引数 `p_memo text default null` を末尾追加＝既存の位置引数8個の呼び出しは非破壊。
**適用済みDBへは本SQLの再実行が必要**（旧8引数シグネチャは明示dropされる）。
...
**memoの永続化（v0.2・レビューHIGH-1対応で配線完了）**: `api/redelivery` のメモ欄（`body.memo`）は
`submitReception` → `register_reception(p_memo)` を通って `reception_requests.memo` に保存される
（500字上限はDB側で強制・非PII照会 `get_reception_public` には出ない）。LINE/SMS/電話チャネルは
現状memo入力を持たないため未指定（null）＝将来チャネル側から供給可能。
