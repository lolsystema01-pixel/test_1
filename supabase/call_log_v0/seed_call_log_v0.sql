...
select public.record_call_log(
  ...
  p_band_key        := 'demo9000',
  p_topic           := '問合せ',
  p_ai_summary      := '配達予定日の確認。本日午前中の見込みと回答。',
...
select public.record_call_log(
  ...
  p_band_key        := 'demo9000',
  p_topic           := '再配達',
  p_ai_summary      := '不在のため翌日午前へ再配達希望。受付登録済み。',
...
select public.record_call_log(
  ...
  p_band_key        := 'demo9000',
  p_topic           := '時間変更',
  p_ai_summary      := '時間帯変更の希望あり。込み入った条件のため担当者から折り返し。',
...
select public.record_call_log(
  ...
  p_band_key        := null,
  p_topic           := 'クレーム',
  p_ai_summary      := '配達時の対応についてお怒りの様子。至急担当者から折り返し希望。',
...
select public.record_call_log(
  ...
  p_band_key        := 'demo9000',
  p_topic           := '置き配',
  p_ai_summary      := '置き配可否の複雑な条件でオペレーターへ転送。',
...
select public.record_call_log(
  ...
  p_topic           := 'その他',
  p_ai_summary      := '通話途中で切断。用件未確定。',
  p_result          := '中断',
  p_priority        := 0
) as seed_7;

-- 確認（件数）------------------------------------------------------------
select result, count(*) from public.call_logs where call_sid like 'CALL-SEED-%' group by result order by result;
