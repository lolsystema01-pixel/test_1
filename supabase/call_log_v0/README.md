...
| `pglite_test.mjs` | E2E検証（37/37 PASS） |
...
### `topic`（用件分類）はcheck制約なしの自由文字列

再配達／時間変更／置き配／問合せ／クレーム／その他 等を想定するが、カテゴリ確定は運用の細かい仕様のため
`result`（AI完結／転送済／折り返し要／中断／いたずら）と`callback_status`（待ち／完了／不要）のみcheck制約を付け、
`topic`は自由記述にした。
