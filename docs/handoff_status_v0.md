...
| `office_init_setup_v0/` | **初期設定 v0.2（§12.14）**：営業所を新規追加したときの**初回ログインでのみ自動表示**する2項目入力（持出バッグリストフォルダURL・ラベルプリンタ機種）。`offices.gdrive_folder_url` を追加（**NULL＝初期設定 未完**の判定に流用・専用フラグ列は作らない・空文字はCHECK禁止）＋保存口 `save_office_init_setup`（SECURITY DEFINER）。フロントは `sort_nav_v0` の `/setup`＋`/home` の初回ゲート | ✅ pglite 30/30＋確認SQL 8/8＋svelte-check 0/0・**実機確認済（5観点OK）**。⚠ ゲートは **`/home` のみ**（URL直打ちで飛ばせる・v0.2の割り切り）。⚠ 権限は「hq＝常時／area＝自営業所かつ初回のみ」＝**業務Aの確認事項**。⚠ 消費側（Gドライブ保存）は未実装＝**URLを入れてもまだ保存されない** |
...
| `pipeline_demo_v0/` | **パイプラインデモ v0 — 🛑 停止済み（2026-07-20・セキュリティ修正）**：`/demo` から配車→採番を動かす一時デモだったが、8関数が **anon 公開の SECURITY DEFINER**（未認証で全営業所のデータ破壊＋PII読取・営業所スコープ判定なし）で**独立2監査が Critical 判定**。`revoke_demo_anon_v0.sql` で **anon/authenticated/public から全面 revoke**（実DB適用済み・実測 anon=0/authenticated=0）＋ソースの grant を RETIRED 化（再実行しても穴が開かない） | 🛑 停止。**`/demo` は動きません**（デモ不要と決定済み）。配車・採番の画面が再度必要になったら**案B**で作り直す＝authenticated のみ＋関数内 `my_office()` スコープ認可＋読取は `security_invoker` ビュー（`record_status_transition`／`office_home_summary` の規約に合わせる） |
...
- `offices(office_code PK, depot_code FK, office_name, dispatch_priority, basket_order, basket_cart_limit, autosave_threshold, request_period_days, auto_logout_enabled, auto_logout_minutes, printer_model, gdrive_folder_url)` … 営業所＋設定。`auto_logout_*`／`printer_model` は `admin_settings_v0`、`gdrive_folder_url` は `office_init_setup_v0` が**モジュールSQLの alter で追加**（`create_schema_v0` には無い＝フレッシュ構築では各モジュールSQLを流す）。いずれも**器のみ・消費側未実装**。書込みは `update_office_settings`（hqのみ）／`save_office_init_setup`（hq＝常時・area＝自営業所かつ初回のみ）経由に限定＝write policy無し。※ `gdrive_folder_url` は **NULL＝初期設定 未完**の判定に流用（専用フラグ列は作らない・空文字はCHECKで禁止）
...
### 7.0.1 ⚠ フレッシュ環境（新規Supabase）を構築するときの注意

**8月末T1本番化で新環境を作る担当者向け。** `address_master` は撤去済みだが、**旧マスタを作る/参照するSQLが各モジュールに残っている**（履歴として残し、実行部は無効化済み）。2026-07-20 時点で以下は**RETIRED表明＋実行部コメントアウト済み**なので、READMEどおりに順に流して問題ない：

`dbschema_v0/`（create_schema・seed_dummy・check_v0）／`master_zoneplan_v0/`（create_master・reset_prev_master・load_master・check_master）／`rls_v0/`（rls_v0・check_rls）／`dispatch_v0/`（dispatch_v0・seed_dispatch）／`ai_status_reply_v0/delivery_status_rpc_v0.sql`／`address_match_v0/match_v0.sql`／`unregistered_address_v0/unregistered_v0.sql`／`csv_import_v0/reset_prev_dummy_v0.sql`

- **住所→共通IDのマスタは `area_master_v0/` を使う**（`master_zoneplan_v0` の Master 部ではない。ZonePlan 部のみ現役）。
- **3関数（`zone_rank`／`dispatch_build`／`delivery_status_public`）は `vocab_fix_v0/migrate_functions_to_area_master_v0.sql` が正**。`dispatch_v0.sql`／`delivery_status_rpc_v0.sql` の定義は**旧版**（RETIRED）なので、そちらを後から流さないこと（④が巻き戻る）。
- 新環境では **②〜⑤（`vocab_fix_v0/`）を通す必要はない**（②〜⑤は既存環境の是正作業。新環境は最初から area_master で構築する）。
- ⚠ `master_zoneplan_v0/reset_prev_master_v0.sql` は `zone_plan` を全削除する。**既存環境では実行しないこと**（②で入れた新語彙1,653件が消える）。
...
- **ドライバーアプリ（T3・`apps/driver_native_v0`）＝RN+Expo（SDK54固定）実装済み・PR#9**：完了/不在タップ→記録口→GPS付きDB記録＋置き配写真POD（`delivery-photos`バケット・自フォルダINSERTのみ・UPDATE/DELETE無し）。検証DBへ **`status_log_v0/record_status_transition_v0.sql`（internal分離版）→`delivery_result_v0`→`delivery_photo_v0` の順で適用済み（2026-07-20・実機E2E済）**。※PR#9レビュー対応の**冪等キー改訂（`client_request_id`）は再適用待ち**（`delivery_result_v0.sql`は適用済みDBへそのまま再実行できる作り）。
- **`delivery_results` の集計ビュー未実装**：管理側の「予定N/実績M」のビュー/UIはまだ無い（別担当）。作る際は**冪等キー適用後のデータであること**を前提にできる＝再送重複は`client_request_id`で原理的に排除済み・`不在`複数行は正当な日内再訪として合算してよい。
